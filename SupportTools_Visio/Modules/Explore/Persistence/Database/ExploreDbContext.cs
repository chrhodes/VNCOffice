...
{
    public class ExploreDbContext : DbContext, IExploreDbContext
    {
        // TODO(crhodes)
        ...
        public ExploreDbContext() : base("Explore_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_CATEGORY);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<ExploreDbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<ExploreDbContext>(new CreateDatabaseIfNotExists<ExploreDbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<ExploreDbContext>(new DropCreateDatabaseIfModelChanges<ExploreDbContext>());

            Database.SetInitializer<ExploreDbContext>(
                new DropCreateDatabaseIfModelChanges<ExploreDbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<ExploreDbContext>(new DropCreateDatabaseAlways<ExploreDbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<ExploreDbContext>(new ExploreDbContextDatabaseInitializer());
        ...
        public override int SaveChanges()
        {
            ...
            try
            {
                ...
                Log.PERSISTENCE($"(SupportTools_VisioDbContext) Exit ({result})", Common.LOG_CATEGORY, startTicks);
        ...
        {
            Int64 startTicks = Log.PERSISTENCE("(ExploreDbContext) Enter", Common.LOG_CATEGORY);
            ...
            try
            {
                ...
                Log.PERSISTENCE($"(ExploreDbContext) Exit ({result})", Common.LOG_CATEGORY, startTicks);
