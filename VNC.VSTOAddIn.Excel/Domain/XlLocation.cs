...
namespace VNC.VSTOAddIn.Excel.Domain
{
    public class XlLocation
    {
        ...
        {
            workSheet = ws;
            startRange = ws.Cells[row, column] as Range;
        ...
        public void CreateTable(string tableName)
        {
            ...
            {
                // You got this error because the C# compiler cannot resolve the correct overload for the Range indexer
                // when using dynamic or COM interop types, and it tries to use the C# dynamic binder,
                // which requires the Microsoft.CSharp assembly.
                // This might be happening because the arguments to ws.Range[...] are not explicitly typed as object,
                // so the compiler cannot resolve the correct method signature and falls back to dynamic binding,
                // which is not supported in your current project setup.
                // Here's how I fixed the code: I explicitly cast the ws.Cells[...] arguments to object when calling ws.Range[object, object].
                // This ensures the correct overload is used and avoids the need for dynamic binding, resolving the CS0656 error.
                ListObject table = ws.ListObjects.Add(XlListObjectSourceType.xlSrcRange,
                    ws.Range[
                        (object)ws.Cells[TableStartRow, TableStartColumn],
                        (object)ws.Cells[TableEndRow, TableEndColumn]],
                    Type.Missing,
                    XlYesNoGuess.xlYes);
        ...
        {
            nextRange = workSheet.Cells[row, nextRange.Column] as Range;
        }

        public void SetColumn(int column)
        {
            nextRange = workSheet.Cells[nextRange.Row, column] as Range;
        }

        public void SetLocation(int row, int column)
        {
            nextRange = workSheet.Cells[row, column] as Range;
        }
