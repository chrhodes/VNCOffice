...
namespace VNCShapeSheetApplication.Actions
{
    internal class Visio_Page
    {
        ...
        {
            Common.WriteToDebugWindow($"{MethodInfo.GetCurrentMethod().Name} Name:>{page.Name}< NameU:>{page.NameU}<", true);
        ...
        public static void CreatePageForShape(Application app, string doc, string page, string shape, string shapeu, string[] args)
        {
            ...
            if (args.Count() != 3)
            {
                Common.WriteToDebugWindow($"Incorrect Argument Count {args.Count()}, expected 3.  Check ShapeSheet", true);
            }
            else
            {
                prefix = args[0];
            ...
            Common.WriteToDebugWindow($"{MethodInfo.GetCurrentMethod().Name}() prefix:>{prefix}< delimiter:>{delimiter}< backgroundPageName:>{backgroundPageName}<", true);
            ...
            try
            {
                ...
                Common.WriteToDebugWindow($"  Shape(Name:>{activeShape.Name}< Text:>{activeShape.Text}< Characters:>{activeShape.Characters.TextAsString}<", true);
                ...
                if (activeShape.CellExistsU["Prop.ReturnLink", 0] != 0)
                {
                    ...
                    Common.WriteToDebugWindow($"  returnLinkProp:>{returnLinkProp}< stencilName:>{stencilName}< shapeName:>{shapeName}< ", true);
                    ...
                    try
                    {
                        ...
                        {
                            Common.WriteToDebugWindow(string.Format("  Cannot find Master named:>{0}<", shapeName), true);
                        }
                    }
                    catch (Exception ex)
                    {
                        Common.WriteToDebugWindow(string.Format("  Cannot find open Stencil named:>{0}<", stencilName), true);
                    }
                }
        ...
        public static void UpdateLayer(Application app, string doc, string page, string shape, string shapeu)
        {
            ...
            foreach (Layer layer in activePage.Layers)
            {
                try
                {
                    Common.WriteToDebugWindow(layer.Name, true);
        ...
        {
            Common.WriteToDebugWindow($"{MethodInfo.GetCurrentMethod().Name}() Page:{pageName} Background:{backgroundPageName}", true);
            ...
            Common.WriteToDebugWindow($"  currentPageIndex:{currentPageIndex}", true);
            ...
            {
                Common.WriteToDebugWindow($"Cannot Find Background Page ({backgroundPageName})", true);
            }
